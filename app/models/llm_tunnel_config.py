...
class LLMTunnelConfig(Base):
    ...
    provider_type = Column(String, default="ngrok", nullable=False)  # For future multi-provider support
    tunnel_url = Column(String, nullable=False)  # e.g., "https://abc123.ngrok.io"
    tunnel_auth_token_encrypted = Column(String, nullable=True)  # Encrypted ngrok auth token or None
    ollama_model_default = Column(String, default="qwen3:8b", nullable=False)  # Default model to use
    request_timeout_sec = Column(Integer, default=30, nullable=False)  # HTTP request timeout
    is_active = Column(Boolean, default=True, nullable=False, index=True)
    
    # Connectivity monitoring
    last_connectivity_check = Column(DateTime, nullable=True)
    last_error = Column(Text, nullable=True)  # Last error message if connectivity failed
