...
def create_action_request(
    ...
    return create_action_request_for_user(db, current_user=current_user, body=body)
...
def approve_action_request(
    ...
    return approve_action_request_for_user(
        db,
...
def reject_action_request(
    ...
    return reject_action_request_for_user(
        db,
        request_id=request_id,
        body=body,
        current_user=current_user,
...
async def reconcile_action_executions(
    ...
    if body.execution_id is not None:
        ...
        cutoff = utc_now() - timedelta(hours=body.lookback_hours)
        query = query.filter(
...
async def mockbank_lifecycle_webhook(
    ...
    if not verify_mockbank_signature(
        body=body,
