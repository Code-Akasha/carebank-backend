...
admin_r = test('post', '/api/auth/login', json={'email': 'admin@carebank.com', 'password': 'Admin1234!'})
user_r = test('post', '/api/auth/login', json={'email': 'jefino@carebank.com', 'password': 'Pass1234!'})
...
if admin_r and admin_r.status_code == 200:
    ...
    if ur and ur.status_code == 200:
        d = ur.json()
        print('  admin/users total: ' + str(d.get('total')) + ' keys: ' + str(list(d.keys())))
...
if user_r and user_r.status_code == 200:
    ...
    if txr and txr.status_code == 200:
        d = txr.json()
        print('  Transactions: total=' + str(d.get('total')) + ' count=' + str(len(d.get('transactions', []))))
    test('get', '/api/health-score/', headers=uh)
    sr = test('post', '/api/simulate', json={'user_id': 'user_001', 'expense_amount': 5000, 'category': 'food'}, headers=uh)
    if sr and sr.status_code == 200:
        d = sr.json()
        impact = d.get('impact', {})
        print('  Simulate: risk=' + str(impact.get('risk_level')) + ' retained=' + str(impact.get('retained_percentage')))
    print('--- AI Chat Test ---')
    cr = test('post', '/api/chat', json={'message': 'What is my balance?', 'user_id': 'user_001', 'thread_id': 'user_001'}, headers=uh)
